...
- 判定結果エリアに Leaflet.js（OpenStreetMap）による測定地点確認マップを追加（物件・室内モード・土地モード両方）：`initResultMap(lat, lng)` / アンバーピンアイコン・10mサークル・zoom19・scrollWheelZoom無効
- マップ直下に「Googleマップで開く」リンクボタンを追加（`initResultMap`内で`el.parentNode.insertAdjacentElement('afterend', gmapBtn)`）
...
- 樹木セクション（`obs-tree-{i}`）に建物セクションと同一の左右トグルUIを追加
  - 方角バッジ（`obst-tree-dir-label-{i}`）＋「任意・精度向上」バッジを追加
  - `side-tree-toggle-{i}` / `side-tree-panel-{i}` / `side-tree-left-label-{i}` / `side-tree-right-label-{i}` を追加
  - `toggleSideTreeObstruction(i)` / `setWinSideTreeH(i, side, val)` 関数追加
  - `makeDefaultWindow` に `sideTreeObst: { left: 0, right: 0 }` 追加
  - `setWinDir` に樹木側ラベル更新ロジック追加（`obst-tree-dir-label` / `side-tree-toggle-label` / 左右ラベル）
  - `setWinObstructionType` に `side-tree-toggle` の表示制御追加（tree/both のみ表示）
  - `calcWindowHeatmap` に `sideTreeObst` の左右遮蔽補正を追加（建物側と同ロジック）
- 遮蔽物セクションのデザイン変更：オレンジ枠ボックス→外出し `<label>` 見出し＋白背景カード
...
| モード | 精度 | 対象ユーザー |
|---|---|---|
| Lite版・図面モード | ★☆☆☆☆ | 内見前・手軽に確認したい・写真撮影できない |
| Lite版・現地撮影モード | ★★★☆☆ | 内見後・現地で正確に判定したい・一般ユーザー |
| Pro版・全部入り | ★★★★★ | 最高精度・不動産業者・BtoB向け |
...
- 遮蔽物（任意・手動）
- 写真なし
- 判定結果：総合スコア・季節別・ヒートマップ・断面シミュレーション
...
**Pro版・全部入り（モード選択不要・図面も写真も両方使う）**
- 緯度経度・住所（手動）
- 不動産広告・図面をアップ → 階数・方角・バルコニーをAI自動認識
- 遮蔽物（任意・手動）
- 窓からの写真3方向（任意）→ AIが遮蔽仰角・SVFを自動解析
- 判定結果：総合スコア・季節別・ヒートマップ・断面シミュレーション・AI診断レポート・時間帯別おすすめ活用法

### Pro版のオンオフ（フェーズ1）
- index.html内の `const PRO_MODE = false;` を `true` に変更してgit pushするだけ
- フェーズ2：URLパラメータ（?pro=true）で切り替え
- フェーズ3：ログイン・課金・回数管理と組み合わせ

### Pro版の実装TODO
- [ ] index.htmlに `const PRO_MODE = false;` の定数を追加
- [ ] PRO_MODEがtrueの場合、Claude APIで不動産広告・図面を解析
- [ ] PRO_MODEがtrueの場合、写真をClaude APIで解析（SVF・遮蔽仰角を自動推定）
- [ ] Pro版専用の判定結果UI（AI診断レポート・時間帯別おすすめ活用法）を追加
